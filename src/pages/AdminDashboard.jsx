                        ...
                        <tr key={i} className="hover:bg-slate-50 transition-all cursor-pointer group">
                          ...
                          <td className="p-4">
                            <span className={`text-[9px] font-black px-2 py-1 rounded-md uppercase ${
                              v.Status === 'EM SERVIÇO' ? 'bg-emerald-100 text-emerald-700' : 
                              v.Status === 'MANUTENÇÃO' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'
                            }`}>
                              {v.Status}
                            </span>
                          </td>
                          ...
                          <td className="p-4 text-right">
                             <button 
                              onClick={() => setSelectedVtr(v)}
                              className="p-2 hover:bg-slate-900 hover:text-white rounded-lg text-slate-400 transition-all"
                             >
                                <History size={18} />
                             </button>
                          </td>
...
const VtrDetailsModal = ({ vtr, onClose, checkOil, onAction }) => {
  ...
  const percentualOleo = Math.max(0, 100 - (kmRodadoPosTroca / 10000) * 100);
...
const MenuBtn = ({ active, onClick, icon, label }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center gap-3 p-3 rounded-xl text-[11px] font-black uppercase transition-all ${active ? 'bg-amber-500 text-slate-900 shadow-lg' : 'text-slate-500 hover:bg-slate-800'}`}
  >
    {icon} {label}
  </button>
);
