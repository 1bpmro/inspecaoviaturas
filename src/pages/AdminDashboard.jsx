    ...
    <div className="min-h-screen bg-slate-50 flex font-sans print:bg-white text-slate-900 relative">
      ...
      `}>
        <div className="p-6 border-b border-slate-800 text-center relative">
          ...
          <p onClick={() => setQruAtivo(!qruAtivo)} className={`text-[9px] font-bold tracking-widest uppercase cursor-pointer select-none transition-all duration-500 hover:text-amber-500 ${qruAtivo ? 'text-amber-400 scale-110' : 'text-slate-500'}`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-white h-auto py-3 md:h-16 border-b border-slate-200 flex flex-col md:flex-row items-center justify-between px-4 md:px-8 shrink-0 print:hidden gap-3">
          ...
          {/* NOVO: SELETOR DE FILTRO POR CARROCERIA */}
          <div className="flex items-center gap-2 w-full md:w-auto">
            <Filter size={14} className="text-blue-600" />
            <select 
              className="bg-slate-100 border border-slate-200 rounded-xl px-3 py-2 text-[10px] font-black uppercase outline-none focus:border-blue-500 w-full md:w-48"
              value={filterCarroceria}
              onChange={(e) => setFilterCarroceria(e.target.value)}
            >
              {LISTA_CARROCERIAS.map(tipo => (
                <option key={tipo} value={tipo}>{tipo === 'TODOS' ? '🚀 TODAS CARROCERIAS' : tipo}</option>
              ))}
            </select>
            <button onClick={handlePrint} className="p-2 md:px-4 md:py-2 bg-slate-900 text-white rounded-lg text-[10px] font-black uppercase flex items-center gap-2 shrink-0">
               <Printer size={16}/>
            </button>
          </div>
        </header>
          ...
          {activeTab === 'frota' && (
            <div className="space-y-4 animate-in fade-in duration-500">
               <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <StatCard label="Resultado Filtro" value={viaturasFiltradas.length} color="blue" />
                <StatCard label="Em Serviço" value={viaturasFiltradas.filter(v => v.Status === 'EM SERVIÇO').length} color="emerald" />
                <StatCard label="Manutenção" value={viaturasFiltradas.filter(v => v.Status === 'MANUTENÇÃO').length} color="red" />
                <StatCard label="Óleo/Atenção" value={viaturasFiltradas.filter(v => checkOil(v).level > 0).length} color="amber" />
              </div>
                      ...
                      <tr key={i} className="hover:bg-blue-50/50">
                        ...
                        <td className="p-5">
                          <div className="flex justify-center gap-2">
                            <button onClick={() => handleEditClick(v)} className="p-2 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition-colors"><Edit2 size={16}/></button>
      ...
      {isAddingVtr && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-[250] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
      ...
      <div className="w-full max-w-lg bg-white h-full shadow-2xl p-6 md:p-10 animate-in slide-in-from-right duration-500 overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
            ...
            <p className="text-[11px] font-bold text-slate-500 mt-1">Tipo: {vtr.TipoCarroceria || 'NÃO DEFINIDO'}</p>
          </div>
